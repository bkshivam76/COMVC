...
namespace WhatsappBusiness.CloudApi
{
    ...
    {
        private static ILog log = LogManager.GetLogger("WhatsAppBusinessClient");
        private readonly HttpClient _httpClient = new HttpClient();
        readonly Random jitterer = new Random();
        private readonly JsonSerializer _serializer = new JsonSerializer();
        private readonly WhatsAppBusinessCloudApiConfig _whatsAppConfig = new WhatsAppBusinessCloudApiConfig();
        ...
        /// <summary>
        /// Send Dynamic URL Button Message
        /// </summary>
        /// <param name="dynamicURLButtonMessage"></param>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public Task<WhatsAppResponse> SendDynamicURLButtonMessage(DynamicURLTemplateMessageRequest dynamicURLButtonMessage, CancellationToken cancellationToken)
        {
            var formattedWhatsAppEndpoint = WhatsAppBusinessRequestEndpoint.SendMessage.Replace("{{Phone-Number-ID}}", _whatsAppConfig.WhatsAppBusinessPhoneNumberId);
            return  WhatsAppBusinessPostAsync<WhatsAppResponse>(dynamicURLButtonMessage, formattedWhatsAppEndpoint, cancellationToken);
        }
        /// <summary>
        /// Send Dynamic URL Button Message Async
        /// </summary>
        /// <param name="dynamicURLButtonMessage"></param>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async Task<WhatsAppResponse> SendDynamicURLButtonMessageAsync(DynamicURLTemplateMessageRequest dynamicURLButtonMessage, CancellationToken cancellationToken = default)
        {
            var formattedWhatsAppEndpoint = WhatsAppBusinessRequestEndpoint.SendMessage.Replace("{{Phone-Number-ID}}", _whatsAppConfig.WhatsAppBusinessPhoneNumberId);
            return await WhatsAppBusinessPostAsync<WhatsAppResponse>(dynamicURLButtonMessage, formattedWhatsAppEndpoint, cancellationToken);
        }
        ...
        private async Task<T> WhatsAppBusinessPostAsync<T>(object whatsAppDto, string whatsAppBusinessEndpoint, CancellationToken cancellationToken = default) where T : new()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var json1 = new JsonTextReader(reader);
                result = _serializer.Deserialize<T>(json1);
            }
            else
            {
                log.Info("API call is failed, please check error details: "+ response.Content);
                ...
                log.Error(json1);
                result = _serializer.Deserialize<T>(json1);
                //whatsAppErrorResponse = _serializer.Deserialize<WhatsAppErrorResponse>(json1);
