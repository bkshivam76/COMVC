...
Public Class Frm_Ledger_Report
...
#Region "Start--> Form Events"
    ...
    Private Sub Form_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        xPleaseWait.Show("Ledger Report" & vbNewLine & vbNewLine & "L o a d i n g . . . !")
        Base = MainBase
        ''/...FOR PROGRAMMING MODE ONLY......\
        Programming_Testing()
        ''\................................../
        Set_Default() ' Prepare Status-bar help text of all objects
        Me.GLookUp_LedgerList.Focus()
    End Sub
...
#Region "Start--> Procedures"
    ...
    Private Sub Set_Default()
        ...
        GridDefaultProperty()
        LookUp_GetLedgerList()
        If LedgerID.Length > 0 Then Grid_Display()
    ...
    Public Sub Grid_Display()
        If GLookUp_LedgerList.Tag Is Nothing Then Return
        If GLookUp_LedgerList.Tag.ToString.Length = 0 Then Return
        ...
        Dim Opening As Double = -1
        Dim oParam As Common_Lib.RealTimeService.Param_GetLedgerOpeningBalance = New Common_Lib.RealTimeService.Param_GetLedgerOpeningBalance
        oParam.OnDate = FrDate
        oParam.InsttId = Base._open_Ins_ID
        oParam.Led_ID = GLookUp_LedgerList.Tag
        oParam.YearID = Base._open_Year_ID
        oParam.SubLedID = Me.GLookUp_LedgerListView.GetRowCellValue(Me.GLookUp_LedgerListView.FocusedRowHandle, "Sub_Led_ID").ToString
        Opening = Base._Reports_Ledgers_DBOps.GetLedgerOpening(oParam)
        'If Opening < 0 Then
        '    Base.HandleDBError_OnNothingReturned()
        '    Exit Sub
        'End If

        Dim param As Common_Lib.RealTimeService.Param_GelLedgerReport = New Common_Lib.RealTimeService.Param_GelLedgerReport
        param.FromDate = FrDate
        param.ToDate = ToDate
        param.Led_ID = GLookUp_LedgerList.Tag
        param.InsttId = Base._open_Ins_ID
        param.YearID = Base._open_Year_ID
        param.SubLedID = Me.GLookUp_LedgerListView.GetRowCellValue(Me.GLookUp_LedgerListView.FocusedRowHandle, "Sub_Led_ID").ToString
        Dim _Party_Table As DataTable = Base._Reports_Ledgers_DBOps.GetLedgerReport(param)
        ...
        ' Me.GridView1.Columns("Voucher").Visible = False
        Me.GridView1.Columns("Date").Width = 70
        Me.GridView1.Columns("Particulars").Width = 165
        Me.GridView1.Columns("Item Name").Width = 140
        'Me.GridView1.Columns("Party").Width = 143
        Me.GridView1.Columns("Debit").Width = 70
        Me.GridView1.Columns("Credit").Width = 70
        Me.GridView1.Columns("Balance").Width = 90
        xPleaseWait.Hide()
    End Sub
    ...
    Public Sub DataNavigation(ByVal Action As String)
        Select Case Action
            Case "REFRESH"
                Grid_Display()
            Case "DETAIL"
                If Me.GridView1.GetRowCellValue(Me.GridView1.FocusedRowHandle, "TR_M_ID") Is Nothing Then Exit Sub
            ...
            Case "PRINT-LIST"
                If Me.GridView1.RowCount > 0 Then
                    Base.Show_ListPreview(GridControl1, Me.Text, Me, True, Printing.PaperKind.A4, GLookUp_LedgerList.Text, Base._open_UID_No, BE_View_Period.Text, True)
                End If
                Me.GridView1.Focus()
            Case "FILTER"
    ...
    '1.GLookUp_LedgerList
    Private Sub GLookUp_LedgerList_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles GLookUp_LedgerList.ButtonClick
        If e.Button.Index = 1 Then
            If e.Button.Tag = "ON" Then
                ...
                Me.GLookUp_LedgerListView.OptionsView.ShowAutoFilterRow = False
                Me.GLookUp_LedgerListView.OptionsCustomization.AllowFilter = False
                Me.GLookUp_LedgerList.ShowPopup()
            Else
                e.Button.Tag = "ON" : e.Button.Caption = "Filter : On" : e.Button.Kind = DevExpress.XtraEditors.Controls.ButtonPredefines.Undo
                Me.GLookUp_LedgerListView.OptionsView.ShowAutoFilterRow = True
                Me.GLookUp_LedgerListView.OptionsCustomization.AllowFilter = True
                Me.GLookUp_LedgerList.ShowPopup()
            End If
        End If
    End Sub
    Private Sub GLookUp_LedgerList_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GLookUp_LedgerList.KeyDown
        'If e.KeyCode = Keys.PageUp Then SendKeys.Send("+{TAB}")
        If e.KeyCode = Keys.PageUp And GLookUp_LedgerList.IsPopupOpen = True Then
            e.SuppressKeyPress = True
            GLookUp_LedgerList.CancelPopup()
            ' Hide_Properties()
            'Me.Txt_No.Focus()
        ElseIf e.KeyCode = Keys.PageUp And GLookUp_LedgerList.IsPopupOpen = False Then
            e.SuppressKeyPress = True
            ' Hide_Properties()
            'Me.List_Lang.Focus()
        End If
    ...
    End Sub
    Private Sub GLookUp_LedgerList_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GLookUp_LedgerList.EditValueChanged
        If Me.GLookUp_LedgerList.Properties.Tag = "SHOW" Then
            If Me.GLookUp_LedgerListView.RowCount > 0 And Val(Me.GLookUp_LedgerListView.FocusedRowHandle) >= 0 Then
                Me.GLookUp_LedgerList.Tag = Me.GLookUp_LedgerListView.GetRowCellValue(Me.GLookUp_LedgerListView.FocusedRowHandle, "ID").ToString.Split(";")(0)
            End If
        Else
        End If
    End Sub
    Private Sub LookUp_GetLedgerList()
        Dim d1 As DataTable = Base._Reports_Ledgers_DBOps.GetLedgerList()
        ...
        If dview.Count > 0 Then
            Me.GLookUp_LedgerList.Properties.ValueMember = "ID"
            Me.GLookUp_LedgerList.Properties.DisplayMember = "Name"
            Me.GLookUp_LedgerList.Properties.DataSource = dview
            Me.GLookUp_LedgerListView.RefreshData()
            Me.GLookUp_LedgerList.Properties.Tag = "SHOW"
            Me.GLookUp_LedgerList.Properties.ReadOnly = False
        Else
            Me.GLookUp_LedgerList.Properties.Tag = "NONE"
        End If
    End Sub
