Public Class Vehicle_Report
    ...
    Private Sub XtraReport1_BeforePrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles Me.BeforePrint
        ...
        '
        '---------------------------------------------------------------
        Dim param As Common_Lib.RealTimeService.Param_GetVehicleReport = New Common_Lib.RealTimeService.Param_GetVehicleReport
        ...
        Dim VehDS As DataSet = MainBase._Reports_Ledgers_DBOps.GetVehicleData(param)
        If VehDS Is Nothing Then
            ...
            Exit Sub
        End If
        For Each XRow In VehDS.Tables(0).Rows
            Vehicle_Data1.Tables("Default_Table").ImportRow(XRow)
        Next
        XrOpenAmt.Text = VehDS.Tables(1).Rows(0)("Opening").ToString()
        XrTotOpening.Text = VehDS.Tables(1).Rows(0)("Opening").ToString()

        XrAddAmt.Text = VehDS.Tables(1).Rows(0)("Addition").ToString()
        XrTotAdd.Text = VehDS.Tables(1).Rows(0)("Addition").ToString()

        XrDedAmt.Text = VehDS.Tables(1).Rows(0)("Deduction").ToString()
        XrTotDed.Text = VehDS.Tables(1).Rows(0)("Deduction").ToString()

        XrCloseAmt.Text = VehDS.Tables(1).Rows(0)("Closing").ToString()
        XrTotClose.Text = VehDS.Tables(1).Rows(0)("Closing").ToString()

        XrOpenQty.Text = VehDS.Tables(2).Rows(0)("OpeningQty").ToString()
        XrAddQty.Text = VehDS.Tables(2).Rows(0)("AdditionQty").ToString()
        XrDedQty.Text = VehDS.Tables(2).Rows(0)("DeductionQty").ToString()
        XrCloseQty.Text = VehDS.Tables(2).Rows(0)("ClosingQty").ToString()
        'xPleaseWait.Hide()
    End Sub
