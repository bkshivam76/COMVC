...
Public Class Frm_Voucher_Win_Gen_Pay_Item
...
#Region "Start--> Form Events"
    ...
    Private Sub Form_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        If iSpecific_ItemID.Length > 0 Then
...
#Region "Start--> Button Events"
    ...
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        ...
        If (keyData = (Keys.Control Or Keys.A)) Then ' Add Party / Person
            ...
            Return (True)
        End If
        If (keyData = (Keys.Control Or Keys.L)) Then ' Show Party Ledger
            BUT_SHOW_LEDGER_Click(Nothing, Nothing)
            Return (True)
        End If
    ...
    Public GS_DESC_MISC_ID As String : Public GS_ITEM_WEIGHT As Decimal
    Public AI_TYPE As String : Public AI_MAKE As String : Public AI_MODEL As String : Public AI_SERIAL_NO As String : Public AI_PUR_DATE As String : Public AI_WARRANTY As Double : Public AI_IMAGE As Byte()
    Public LS_NAME As String : Public LS_BIRTH_YEAR As String : Public LS_INSURANCE, LS_INSURANCE_ID, LS_INS_POLICY_NO, LS_INS_DATE As String : Public LS_INS_AMT As Double
    ...
    Public LB_PRO_TYPE, LB_PRO_CATEGORY, LB_PRO_USE, LB_PRO_NAME, LB_PRO_ADDRESS, LB_ADDRESS1, LB_ADDRESS2, LB_ADDRESS3, LB_ADDRESS4, LB_STATE_ID, LB_DISTRICT_ID, LB_CITY_ID, LB_PINCODE, LB_OWNERSHIP, LB_OWNERSHIP_PARTY_ID, LB_SURVEY_NO, LB_CON_YEAR, LB_RCC_ROOF, LB_PAID_DATE, LB_PERIOD_FROM, LB_PERIOD_TO, LB_DOC_OTHERS, LB_DOC_NAME, LB_OTHER_DETAIL, LB_REC_ID As String : Public LB_TOT_P_AREA, LB_CON_AREA, LB_DEPOSIT_AMT, LB_MONTH_RENT, LB_MONTH_O_PAYMENTS As Double : Public LB_DOCS_ARRAY, LB_EXTENDED_PROPERTY_TABLE As DataTable : Public LB_REC_EDIT_ON As DateTime
    Public X_TP_ID, TP_BILL_NO, TP_BILL_DATE, TP_PERIOD_FROM, TP_PERIOD_TO As String : Public Ref_RecID As String
    ...
    Private Sub BUT_SAVE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BUT_SAVE_COM.Click
        ...
        If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._Edit Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New_From_Selection Then
            ...
            '.................................................................................
            'If iProfile = "LAND & BUILDING" Or iProfile = "OTHER ASSETS" Then
            ...
            If Me.GLookUp_ItemListView.FocusedRowHandle < 1 Then
            ...
            If GLookUp_PurListView.FocusedRowHandle < 1 Then
                Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
        ...
        'If GLookUp_ItemList.Tag = "94fc22f1-6ee4-43c2-ba9d-6e723a7dc563" Then ' And Base._open_Year_ID > "1415"
        '    Dim xfrom_TDs_ded As New Frm_Voucher_Win_Gen_Pay_Tds_Ded
        '    xfrom_TDs_ded.MainBase = Base : xfrom_TDs_ded.xMID = iTxnM_ID : xfrom_TDs_ded.lblMentionedInvoucher.EditValue = Me.Txt_Amt.EditValue
        ...
        '    End If
        '    TDS_Deduction_List = xfrom_TDs_ded.TDS_Deduction_List
        'End If
        '+-----------------END------------------+
        ...
        If iVoucher_Type.Trim.ToUpper = "LAND & BUILDING" And Not iProfile.ToUpper = "LAND & BUILDING" Then ' L&B Expense Item
            ...
            If Me.Txt_Amt.Properties.ReadOnly = True Then
                xfrm.GridControl1.Enabled = False
                xfrm.Text += "(Referenced Property has been sold/transferred)"
            End If

            xfrm.ShowDialog(Me)
            If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
                'New selection is considered only when refrenced property is valid one (not sold/tf)
                If Me.Txt_Amt.Properties.ReadOnly = False Then LB_REC_ID = xfrm.LB_REC_ID
                LB_REC_EDIT_ON = xfrm.REC_EDIT_ON 'added for multiuser check
                If xfrm.closing_value + Val(Txt_Amt.Text) < 0 Then
                    Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
                    Me.ToolTip1.Show("Sorry !  Updating of Construction Amount creates a Negative Closing Balance in Current Year for Property( " & xfrm.lb_name & " )", Me.Txt_Amt, 0, Me.Txt_Amt.Height, 5000)
                    Me.Txt_Amt.Focus()
                    Me.DialogResult = Windows.Forms.DialogResult.None
                    Exit Sub
                End If
        ...
        If iProfile = "WIP" Then
            ...
            If xFrm.DialogResult = Windows.Forms.DialogResult.OK Then
                If xFrm.RadioGroup1.SelectedIndex = 0 Then
                    ...
                    xFrm1.Tag = Me.Tag
                    If Me.Txt_Amt.Properties.ReadOnly = True Then
                        xFrm1.GridControl1.Enabled = False
                        xFrm.Text += "(Referenced Property has been sold/transferred)"
                    End If

                    xFrm1.ShowDialog(Me)
                    If xFrm1.DialogResult = Windows.Forms.DialogResult.OK Then
                        If xFrm1.Closing + Val(Txt_Amt.Text) < 0 Then
                            Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
                        ...
                        If Base._next_Unaudited_YearID <> Nothing Then
                            If xFrm1.NextYearClosing + Val(Txt_Amt.Text) < 0 Then
                                Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
                        ...
                        'check of remaining balance in previously refered WIP in case the reference has been changed 
                ...
                Else
                    ...
                    If Me.Txt_Amt.Properties.ReadOnly = True Then
                        xFrm2.Txt_Amount.Properties.ReadOnly = True
                        xFrm2.Txt_Amount.ToolTip = "Created WIP has been referenced in other entries"
                    End If
                    xFrm2.ShowDialog(Me)
        ...
        Select Case iProfile.ToUpper
            Case "TELEPHONE BILL"
                Dim xfrm As New Frm_Telephone_Select
                xfrm.Text = Me.Text & " (Telephone Detail)..."
                ...
                If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._Edit Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._View Then
                    ...
                    Else
                        ...
                        End If
                        xfrm.xID.Text = X_TP_ID
                        xfrm.TP_BILL_NO = TP_BILL_NO
                        xfrm.TP_BILL_DATE = TP_BILL_DATE
                        xfrm.TP_PERIOD_FROM = TP_PERIOD_FROM
                        xfrm.TP_PERIOD_TO = TP_PERIOD_TO
                    End If
                End If
                xfrm.xLeft = Me.Left + 88 : xfrm.xTop = Me.Top + 182
                xfrm.ShowDialog(Me)
                If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
                    X_TP_ID = xfrm.GLookUp_TeleList.Tag
                    TP_BILL_NO = xfrm.Txt_Bill_No.Text
                    If IsDate(xfrm.Txt_Bill_Date.Text) Then
                        TP_BILL_DATE = xfrm.Txt_Bill_Date.DateTime
                    End If
                    If IsDate(xfrm.Txt_Fr_Date.Text) Then
                        TP_PERIOD_FROM = xfrm.Txt_Fr_Date.DateTime
                    End If
                    If IsDate(xfrm.Txt_To_Date.Text) Then
                        TP_PERIOD_TO = xfrm.Txt_To_Date.DateTime
                    End If
            ...
            Case "GOLD", "SILVER"
                ...
                If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._Edit Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._View Then
                    ...
                    Else
                        ...
                        If Me.Txt_Amt.Properties.ReadOnly = True Then
                            xfrm.Txt_Weight.Properties.ReadOnly = True
                            xfrm.Txt_Weight.ToolTip = "Created asset has been referenced in other entries"
                        End If
                    End If
                End If
                xfrm.xLeft = Me.Left + 40 : xfrm.xTop = Me.Top + 55
                ...
                If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
                    ...
                    Me.Txt_Rate.Text = Me.Txt_Amt.Text
            ...
            Case "OTHER ASSETS"
                ...
                If Me.Txt_Amt.Properties.ReadOnly = True Then
                    ...
                    xfrm.Txt_Amt.ToolTip = "Created asset has been referenced in other entries"

                    xfrm.Txt_Qty.Properties.ReadOnly = True
                    xfrm.Txt_Qty.ToolTip = "Created asset has been referenced in other entries"
                End If
                xfrm.ShowDialog(Me)
                If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
            ...
            Case "VEHICLES"
                ...
                If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._Edit Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._View Then
                    ...
                    Else
                        ...
                        xfrm.VI_E_DATE = VI_INS_EXPIRY_DATE
                        xfrm.Txt_Others.Text = Me.Txt_Remarks.Text
                ...
                If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
                    ...
                    Dim _Owner_Free As String = Nothing
            ...
            Case "LAND & BUILDING"
                ...
                If xfrm.DialogResult = Windows.Forms.DialogResult.OK Then
                    ...
                    LB_OTHER_DETAIL = xfrm.Txt_Remarks.Text
    ...
    End Sub
    Private Sub But_PersAdd_Click(sender As Object, e As EventArgs) Handles But_PersAdd.Click
        If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._Edit Or Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New_From_Selection Then
            ...
            xfrm.ShowDialog(Me)
            LookUp_GetPartyList()
            If xfrm.xID.Text.Length > 0 Then
                Me.GLookUp_PartyList.ShowPopup() : Me.GLookUp_PartyList.ClosePopup()
                Me.GLookUp_PartyListView.FocusedRowHandle = Me.GLookUp_PartyListView.LocateByValue("ID", xfrm.xID.Text)
                Me.GLookUp_PartyList.EditValue = xfrm.xID.Text
                Me.GLookUp_PartyList.Properties.Tag = "SHOW"
                Me.GLookUp_PartyList.Properties.ReadOnly = False
                Me.GLookUp_PartyList.Tag = xfrm.xID.Text
            End If
            xfrm.Dispose()
        End If
    End Sub
    Private Sub BUT_SHOW_LEDGER_Click(sender As Object, e As EventArgs) Handles BUT_SHOW_LEDGER.Click
        If GLookUp_PartyListView.FocusedRowHandle >= 0 Then
            Dim xFrm As Frm_Party_Report = New Frm_Party_Report
            xFrm.PartyID = Me.GLookUp_PartyList.Tag
            xFrm.Txt_TitleX.Text = "Current Year Transactions for " + Me.GLookUp_PartyList.Text
            xFrm.Opening = 0 ' Handle openings in case of party ledger being opened from non-party report screen
            xFrm.MainBase = Base
            xFrm.ShowDialog()
        End If
    End Sub
...
#Region "Start--> TextBox Events"
    Private Sub TxtGotFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Txt_Amt.GotFocus, Txt_Amt.Click, Txt_Remarks.GotFocus, Txt_Qty.GotFocus, Txt_Qty.Click, Txt_Rate.GotFocus, Txt_Rate.Click
        Dim txt As TextEdit
        txt = CType(sender, TextEdit)
        If txt.Name = Txt_Amt.Name Or txt.Name = Txt_Qty.Name Or txt.Name = Txt_Rate.Name Then
            txt.SelectAll()
        ElseIf Val(txt.Properties.Tag) = 0 Then
            SendKeys.Send("^{END}") : txt.Properties.Tag = 1
        End If
    End Sub
    Private Sub TextKeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Txt_Amt.KeyPress, Txt_Remarks.KeyPress, TXT_TDS.KeyPress
        If e.KeyChar = "[" Then e.KeyChar = "("
        If e.KeyChar = "]" Then e.KeyChar = ")"
        If e.KeyChar = "'" Then e.KeyChar = "`"
    End Sub
    Private Sub TextKeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Txt_Amt.KeyDown, Txt_Remarks.KeyDown, Txt_Qty.KeyDown, Txt_Rate.KeyDown, TXT_TDS.KeyDown
        Dim txt As TextEdit
        txt = CType(sender, TextEdit)
        Hide_Properties()
    ...
    Private Sub Txt_Qty_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Txt_Qty.EditValueChanged, Txt_Rate.EditValueChanged
        If Calc_Allow Then
            If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New Then Me.Txt_Amt.Text = Val(Txt_Qty.Text) * Val(Txt_Rate.Text)
        End If
    End Sub
    Private Sub TxtComboEditGotFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmb_Unit.GotFocus
        Dim txt As ComboBoxEdit
        txt = CType(sender, ComboBoxEdit)
        'txt.ShowPopup()
        txt.SelectionStart = txt.Text.Length
    End Sub
    Private Sub TxtComboEditKeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Cmb_Unit.KeyDown
        Dim txt As ComboBoxEdit
        txt = CType(sender, ComboBoxEdit)
...
#Region "Start--> Procedures"
    ...
    Private Sub SetDefault()
        ...
        If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._View Then
            ...
            Select Case iProfile.ToUpper
                Case "GOLD", "SILVER", "OTHER ASSETS", "LIVESTOCK", "VEHICLES", "LAND & BUILDING", "TELEPHONE BILL"
            ...
            End Select
        End If
        If Val(Me.Tag) = Common_Lib.Common.Navigation_Mode._New And Base._IsVolumeCenter Then
            Me.GLookUp_PurList.ShowPopup() : Me.GLookUp_PurList.ClosePopup()
            Me.GLookUp_PurListView.FocusedRowHandle = Me.GLookUp_PurListView.LocateByValue("PUR_ID", "8f6b3279-166a-4cd9-8497-ca9fc6283b25")
            ...
            Me.GLookUp_PurList.Tag = "8f6b3279-166a-4cd9-8497-ca9fc6283b25"
    ...
    Private Sub Set_Disable(ByVal SetColor As Color)
        ...
        Me.Cmb_Unit.Enabled = False : Me.Cmb_Unit.Properties.AppearanceDisabled.ForeColor = SetColor
        ...
        Me.BE_TDS_Rate.Enabled = False : Me.BE_TDS_Rate.Properties.AppearanceDisabled.ForeColor = SetColor
        Me.BE_TDS_Section.Enabled = False : Me.BE_TDS_Section.Properties.AppearanceDisabled.ForeColor = SetColor
    ...
    Private Sub FetchExtensionData()
        ...
        With LB_EXTENDED_PROPERTY_TABLE
            ...
            .Columns.Add("LB_REC_ID", Type.GetType("System.String"))
        End With
        Dim LB_Ext As DataTable = Base._Payment_DBOps.GetLandBuildingExtensions(LB_REC_ID)
        If LB_Ext Is Nothing Then
            Base.HandleDBError_OnNothingReturned()
            Exit Sub
        End If
    ...
    Public Sub FetchLBDocuments()
        ...
        With LB_DOCS_ARRAY
            ...
            .Columns.Add("LB_REC_ID", Type.GetType("System.String"))
        End With
        Dim LB_DOC As DataTable = Base._Payment_DBOps.GetLandBuildingDocs(LB_REC_ID)
        If LB_DOC Is Nothing Then
            Base.HandleDBError_OnNothingReturned()
            Exit Sub
        End If
        ...
        Next
    End Sub
    Private Sub GetTDSRate()
        TXT_TDS.Enabled = True
        If GLookUp_ItemListView.FocusedRowHandle <= 0 Or GLookUp_PartyListView.FocusedRowHandle <= 0 Then
            BE_TDS_Rate.Text = "NA"
            TXT_TDS.Enabled = False
            Exit Sub
        End If
        If GLookUp_ItemListView.GetRowCellValue(GLookUp_ItemListView.FocusedRowHandle, "ITEM_TDS_CODE").ToString.ToUpper = "NONE" Then
            BE_TDS_Rate.Text = "NA"
            TXT_TDS.Enabled = False
            Exit Sub
        End If
        Dim SQL_STR1 As String = ""
        Dim PAN As Object = GLookUp_PartyListView.GetRowCellValue(GLookUp_PartyListView.FocusedRowHandle, "PAN")
        Dim d1 As DataTable = Base._Payment_DBOps.GetTDSRate(IIf(PAN Is Nothing, "", PAN.ToString), GLookUp_ItemListView.GetRowCellValue(GLookUp_ItemListView.FocusedRowHandle, "ITEM_TDS_CODE"))
        If d1 Is Nothing Then
            Base.HandleDBError_OnNothingReturned()
            Exit Sub
        End If
        If d1.Rows.Count > 0 Then
            BE_TDS_Rate.Text = d1.Rows(0)(0) & "%"
        End If
...
#Region "Start--> LookupEdit Events"
    ...
    Private Sub GLookUp_ItemList_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GLookUp_ItemList.EditValueChanged
        If Me.GLookUp_ItemList.Properties.Tag = "SHOW" Then
            If Me.GLookUp_ItemListView.RowCount > 0 And Val(Me.GLookUp_ItemListView.FocusedRowHandle) > 0 Then
                ...
                BE_TDS_Section.Text = Me.GLookUp_ItemListView.GetRowCellValue(Me.GLookUp_ItemListView.FocusedRowHandle, "ITEM_TDS_CODE").ToString
                ...
                If iProfile = "VEHICLES" Or iProfile = "LIVESTOCK" Or iProfile = "LAND & BUILDING" Then
                    ...
                    Me.Cmb_Unit.Enabled = False : Me.Cmb_Unit.Text = "NO"
                ...
                ElseIf iProfile = "GOLD" Or iProfile = "SILVER" Or iProfile = "OTHER ASSETS" Then
                    ...
                    Me.Cmb_Unit.Enabled = True : Me.Cmb_Unit.Text = "NO"
                    Me.Txt_Rate.Enabled = True
                Else ' OTHER
                    Me.Txt_Qty.Enabled = True : Me.Txt_Qty.Text = ""
                    Me.Cmb_Unit.Enabled = True
                    Me.Txt_Rate.Enabled = True
                End If

                If (Me.GLookUp_PartyListView.RowCount > 0 And Val(Me.GLookUp_PartyListView.FocusedRowHandle) >= 0) Then
                    If iTDS_CODE.ToUpper <> "NONE" And Me.GLookUp_PartyListView.GetRowCellValue(Me.GLookUp_PartyListView.FocusedRowHandle, "PAN").ToString.Length < 10 Then
                        Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
                        Me.ToolTip1.Show("P A N   N o .   n o t   s p e c i f i e d   i n   P a r t y   D e t a i l s . . . !" & vbNewLine & vbNewLine & "TDS Rates will be shown as higher for parties without PAN information." & vbNewLine & "Please enter PAN information in AddressBook under 'General' Tab.", Me.GLookUp_PartyList, 0, Me.GLookUp_PartyList.Height, 5000)
                    Else
                        Me.ToolTip1.Hide(Me.GLookUp_PartyList)
                    End If
                End If
        ...
        End If
    End Sub
    Private Sub GLookUp_ItemList_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GLookUp_ItemList.Leave
        If Val(Me.GLookUp_ItemListView.FocusedRowHandle) <= 0 Then Me.GLookUp_ItemList.EditValue = Nothing
    End Sub
    Private Sub LookUp_GetItemList()
        Dim d1 As DataTable = Base._Payment_DBOps.GetLedgerItems(Base.Is_HQ_Centre)
        If d1 Is Nothing Then
    ...
    Private Sub LookUp_GetPurposeList()
        Dim d1 As DataTable = Base._Payment_DBOps.GetProjects("PUR_NAME", "PUR_ID")
        If d1 Is Nothing Then
            DevExpress.XtraEditors.XtraMessageBox.Show(Common_Lib.Messages.SomeError, "Error!!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
    ...
    Private Sub GLookUp_PartyList1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GLookUp_PartyList.KeyDown
        ...
        End If
    End Sub
    Private Sub GLookUp_PartyList1_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GLookUp_PartyList.EditValueChanged
        If Me.GLookUp_PartyList.Properties.Tag = "SHOW" Then
            If (Me.GLookUp_PartyListView.RowCount > 0 And Val(Me.GLookUp_PartyListView.FocusedRowHandle) >= 0) Then
                ...
                If Me.GLookUp_ItemListView.FocusedRowHandle >= 0 And iTDS_CODE.ToUpper <> "NONE" And Me.GLookUp_PartyListView.GetRowCellValue(Me.GLookUp_PartyListView.FocusedRowHandle, "PAN").ToString.Length < 10 Then
                    Me.ToolTip1.ToolTipTitle = "Incomplete Information . . ."
                    Me.ToolTip1.Show("P A N  N o.  n o t  s p e c i f i e d  i n  P a r t y  D e t a i l s . . . !" & vbNewLine & vbNewLine & "TDS Rates will be shown as higher for parties without PAN information." & vbNewLine & "Please enter PAN information in AddressBook under 'General' Tab.", Me.GLookUp_PartyList, 0, Me.GLookUp_PartyList.Height, 5000)
                Else
                    Me.ToolTip1.Hide(Me.GLookUp_PartyList)
    ...
    Private Sub LookUp_GetPartyList()
        Dim d1 As DataTable = Base._Payment_DBOps.GetParties("Name", "ID")
        Dim dview As New DataView(d1) : dview.Sort = "Name"
        If dview.Count > 0 Then
            Dim ROW As DataRow : ROW = d1.NewRow() : ROW("Name") = "" : d1.Rows.InsertAt(ROW, 0)
