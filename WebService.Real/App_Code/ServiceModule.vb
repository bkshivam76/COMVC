...
#Region "Param Classes"
        ...
        Public Class Parameter_Update_ServiceModule
            Public firstname As String
            Public lastname As String
            Public fullname As String
            Public verified_email As String
            Public user_image As String
...
#End Region
        ...
        '                                ",SP_OTHER_DETAIL,SP_STATUS,REC_ADD_ON,REC_ADD_BY,REC_EDIT_ON,REC_EDIT_BY,REC_STATUS,REC_STATUS_ON,REC_STATUS_BY,REC_ID,SP_SERVICE_PLACE_NAME,SP_COD_YEAR_ID " &
        ...
        'End Function
        Public Shared Function GetCenters(inBasicParam As ConnectOneWS.Basic_Param) As DataTable
            Dim dbService As ConnectOneWS = New ConnectOneWS()
            Dim onlineQuery As String = " Select CEN_ID, CEN_UID, CEN_NAME, REC_ID from centre_info "
        ...
        ''' <returns></returns>
        ''' <remarks>RealServiceFunctions.ServiceModule_Update</remarks>
        Public Shared Function Update(ByVal UpParam As Parameter_Update_ServiceModule, inBasicParam As ConnectOneWS.Basic_Param, Optional EditTime As DateTime = Nothing) As Boolean
            Dim dbService As ConnectOneWS = New ConnectOneWS()
            Dim OnlineQuery As String = " Update service_users_info SET " &
                                        "FirstName        =" & UpParam.firstname & ", " &
                                        "LastName       ='" & UpParam.lastname & "', " &
                                        "FullName     =" & UpParam.fullname & ", " &
                                        "Verified_Email     ='" & UpParam.verified_email & "', " &
                                        "User_Image           ='" & UpParam.user_image & "', " &
                                         "  WHERE Email    ='" & UpParam.email & "'"

            dbService.Update(ConnectOneWS.Tables.SERVICE_USERS_INFO, OnlineQuery, inBasicParam, EditTime)
            Return True
        End Function
        ...
        '    Dim dbService As ConnectOneWS = New ConnectOneWS()
        '    Dim onlineQuery As String = " SELECT I.INS_SHORT AS 'Institute',C.CEN_NAME AS 'Centre No.',C.CEN_UID AS 'UID',C.CEN_PAD_NO AS 'No.',SP.SP_SERVICE_PLACE_NAME AS 'Service Place Name',SP.SP_SERVICE_PLACE_TYPE AS 'Place Type', SP.REC_ID AS 'SP_ID',SP.REC_EDIT_ON " &
        '                                " FROM Service_place_info as SP  " &
        '                                " INNER JOIN Centre_info AS C ON (SP.SP_CEN_ID=C.CEN_ID) " &
        '                                " INNER JOIN Institute_info AS I ON (C.CEN_INS_ID=I.INS_ID) " &
        '                                " Where SP.REC_STATUS IN (0,1,2) AND C.CEN_BK_PAD_NO='" & BK_PAD_NO & "' " &
        ...
        '                                          "REC_ADD_ON,REC_ADD_BY,REC_EDIT_ON,REC_EDIT_BY,REC_STATUS,REC_STATUS_ON,REC_STATUS_BY,REC_ID, SP_ORG_REC_ID" &
        '                                          ") VALUES(" &
